...
Скрипт:
Принимает конфиг через Hydra
Создаёт Таск для ClearML
Загружает и обрабатывает данные через data_loader.py
Обучает модель (с CV или без)
Логирует метрики и графики в ClearML
Сохраняет препроцессоры и модель
...
Загружает лучшие модели из models/best/train_120/
Извлекает только гиперпараметры (без артефактов fit)
Обучает новые модели на полном датасете (150k строк)
Сохраняет в models/best/train_150/
```
bash
...
Создать конфиг в configs/model/ (по аналогии с lgbm.yaml)
Создать конфиг эксперимента в configs/experiment/
